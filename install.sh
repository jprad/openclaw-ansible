...
# Clawdbot Ansible - Dependency Installer
# This script installs required dependencies (Ansible and collections)
# Run this before using ./run-playbook.sh
...
echo -e "${GREEN}╔════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║   Clawdbot Ansible - Setup             ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════╝${NC}"
echo ""
...
if [ ! -f "playbook.yml" ] || [ ! -f "requirements.yml" ]; then
    echo -e "${RED}✗ Error: Not in clawdbot-ansible directory${NC}"
    echo -e "${RED}  Please run this script from the clawdbot-ansible repository root${NC}"
    exit 1
fi
